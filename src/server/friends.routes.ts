...
export const friendsRouter = new OpenAPIHono()
  ...
  .openapi(CreateFriendRoute, async (c) => {
    ...
    // Only run Sentry test flow if explicitly requested via query param
    const shouldThrowTestError =
      c.req.query("throw") === "1" ||
      c.req.query("throw") === "true" ||
      c.req.query("testError") === "1" ||
      c.req.query("testError") === "true";
